    ...
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for (int i = 2; i < nums.size(); i++) {
            ...
            for (int j = 1; j < nums.size(); j++) {
                ...
                auto target = k.find(-(nums[i] + nums[j]));
                ...
                &&  j != target->second) {
                    ...
                    result.back().emplace_back(nums[j]);
                    result.back().emplace_back(target->first);
                }
                debugln(i, j, nums, nums[i], nums[j], result);
            }   
        }
        return result;
    }
...
int main() {
    ...
    cout << "Input Numbers (separated by ' ' or ',')\n -> " << CYAN;
    getline(cin, text); RESETL

    vector<int> nums = stringToIntegerVector(text);
    debugln  (nums, text);

    print2DVector("Answer", solution.threeSum(nums));
    // sw.start();
    // int time = sw.stop();
