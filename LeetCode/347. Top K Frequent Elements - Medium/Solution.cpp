...
int main() {
    ...
    int much;
    string text;
    cout << "Masukkan Angka untuk di cek elemen terbanyak, dibedakan ',' dan ' '\n -> " << CYAN;
    getline(cin, text); RESETL
    cout << "Banyak elemen yang akan di tampilkan ? " << CYAN;
    cin >> much; RESETL
    ...
    sw.start();
    nums = solution.topKFrequent(nums, much);
    int time = sw.stop();

    outputln ("Elemen yang muncul terbanyak :", GREEN, nums, RESET);
