...
public:
    int sumAll(vi& x) {
        scope("Inside function sumAll");
        // scope() biar lebih mudah di baca di debug

        int sum{0};
        each(it, x) {
            scope("inside for each");
            debug("before", sum, it);
...
#pragma input
    ...
    Redirect(R"(3#10#20#30)");
    /*
    kalau pake debug Redirect diatas harus di isi
    
    # -> itu sebagai pemisah nya, spasi juga bisa
    */
...
#pragma main
    ...
    vi vectors (max);
    input(vectors);
    // input 3 angka dari redirect
    // 10, 20, 30

    debug("Before", max, vectors);
    // buat nampilin data yang mau di cek
    // string di awal debug, itu artinya menjadi judul dari debug tsb

    int sum = s.sumAll(vectors);
    debug("After ", sum, vectors);

    outputln(sum);
    // mirip kaya print() di python
    
    outputln("With delim()    :", delim(vectors, "||"));
    outputln("Without delim() :", vectors);
    // delim() gunanya buat ganti apa yang memisahkan antara indeks di dalam array
