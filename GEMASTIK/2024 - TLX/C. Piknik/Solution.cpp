...
int main() {
    int    max, res, left, right, cars {0}, unique{0};
    input (max, res);
    ...
    for   (int i = 0; i < res; i++) {
        ...
        for (auto& gay : guys) {
            auto lefts  = find(all(gay), left );
            auto rights = find(all(gay), right);
            if (lefts != gay.end() && rights != gay.end()) {
                founded = true;
                break;
            }
            else if (lefts != gay.end()) {
                gay.emplace_back(right);
                founded = true;
                unique++;
                break;
            }
            else if (rights != gay.end()) {
                gay.emplace_back(left);
                founded = true;
                unique++;
                break;
            }
            founded = false;
            // debugln("INSIDE INSIDE", gay);
        }
        if (!founded) {
            cars++;
            guys.emplace_back();
            guys.back().emplace_back(left);
            guys.back().emplace_back(right);
            unique += 2;
        }
        // debugln("Inside", left, right, i, guys);
    }

    if (max - unique > 1) cars++;
