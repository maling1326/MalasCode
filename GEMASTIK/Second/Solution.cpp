...
int main() {
    ...
    int facing[wish.size()];
    for (int i = 0; i < wish.size(); i++) {
        switch (wish[i]) {
        case 'N': facing[i] = 0; break;
        case 'E': facing[i] = 1; break;
        case 'S': facing[i] = 2; break;
        case 'W': facing[i] = 3; break;
        }
    }
    
    rn = facing[0];

    for (int i = 1; i < twirls; i++) {
        bool canOne = false, canTwo = false, canThree = false;

        int left = (rn + 3) % 4;  // equivalent to (rn - 1 + 4) % 4
        int right = (rn + 1) % 4;
        int opp = (rn + 2) % 4;

        if (i < twirls - 1) {
            if (facing[i + 1] == left || facing[i + 1] == right) {
                canOne = true;
            }
        }
        if (i < twirls - 2) {
            if (facing[i + 2] == rn || facing[i + 2] == opp) {
                canTwo = true;
            }
        }
        if (i < twirls - 3) {
            // Define your condition for canThree, example:
            // canThree is true if facing[i + 3] matches rn, left, right, or opp
            int val = facing[i + 3];
            if (val == rn || val == left || val == right || val == opp) {
                canThree = true;
            }
        }
        ...
        if (canThree) {
            rn = facing[i + 3];
            acc++;
            i += 2;  // Skip next two indices because we jumped ahead by 3
        }
        else if (canTwo) {
            rn = facing[i + 2];
            acc++;
            i++;  // Skip next index because we jumped ahead by 2
        }
        else if (canOne) {
            rn = facing[i + 1];
            acc++;
