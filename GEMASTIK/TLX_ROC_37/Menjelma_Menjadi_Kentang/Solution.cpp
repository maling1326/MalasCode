...
#pragma main
    ...
    input(full, half);
    if (accumulate(full.begin(), full.end(), 0) < K) {
        outputln (1);
        return 0;
    }

    int HalfSum = accumulate(all(half), 0);
    // auto min_it = std::min_element(full.begin(), full.end());
    // int lowest_element = *min_it;

    while (true) {
        ++i;
        sums = K * i;
        debug(sums, i, N, K, HalfSum, full, half);
