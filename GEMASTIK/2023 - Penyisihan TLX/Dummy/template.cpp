...
void solve(vi cups) {
        int avg = accumulate(all(cups), 0) / size(cups);
        int index = 0;
        int count = 0;
        pii idx;
        if (all_of(all(cups), [&](int n){
            return n == avg;
            })) output("SESUAI");
        else if (count_if(all(cups), [&](int n){
            count++;
            if (!index)
                index = count;
            else idx = make_pair(index - 1, count - 1);
            return n != avg;
        }) == 2) {
                if (cups[idx.first] + cups[idx.second] == 2 * avg)
                    if (cups[idx.first] > cups[idx.second])
                        output(abs(cups[idx.first] - avg), idx.first + 1, idx.second + 1);
                    else 
                        output(abs(cups[idx.second] - avg), idx.second + 1, idx.first + 1);
                else output("TIDAK SESUAI");
        }
        else output("TIDAK SESUAI");
        debug(avg);
