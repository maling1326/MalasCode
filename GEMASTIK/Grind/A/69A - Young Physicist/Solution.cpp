...
public:
    void testbool(int max) {
        scope("testbool Function");
        for (int i = 0; i < max; i++) {
            scope("Forloop testbool");
            int much;
            input(much);
            switch (isBodyMove(much)) { 
                case true : debugln("YES"); break;
                case false: debugln("NO");  break; 
            }
        }
    };

    bool isBodyMove(int& m) {
        scope("isBodyMove Function");
        int s = 0;
        for (int i = 0; i < (m * 3); ++i) {
            scope("Forloop isBodyMove");
            int temp;
            input(temp);
            s += temp;
            debugln(temp, s);
        ...
        switch (s) { 
            case 0 : debugln("YES"); break;
            default: debugln("NO");  break; 
        }
        return {s == 0};
    }
};
...
_m__main:
    ...
    switch (s.isBodyMove(much)) { case true : outputln("YES"); break;
